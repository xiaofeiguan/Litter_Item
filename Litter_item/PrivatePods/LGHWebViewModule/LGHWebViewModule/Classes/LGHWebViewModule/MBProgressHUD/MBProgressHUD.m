...
static const CGFloat MBDefaultLabelFontSize = 12.f;
...
- (void)commonInit {
    ...
    _animationType = MBProgressHUDAnimationZoomOut;
...
//    _contentColor = [UIColor.darkGrayColor colorWithAlphaComponent:1.];
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    self.label.textColor = wcolor;
    self.detailsLabel.textColor = wcolor;
    [self.button setTitleColor:wcolor forState:UIControlStateNormal];

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    if (self.activityIndicatorColor) {
        wcolor = self.activityIndicatorColor;
    }
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = wcolor;
        }
        ...
        if (appearance.progressTintColor == nil) {
            ((MBRoundProgressView *)indicator).progressTintColor = wcolor;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MBRoundProgressView *)indicator).backgroundTintColor = [wcolor colorWithAlphaComponent:0.1];
        }
        ...
        if (appearance.progressColor == nil) {
            ((MBBarProgressView *)indicator).progressColor = wcolor;
        }
        if (appearance.lineColor == nil) {
            ((MBBarProgressView *)indicator).lineColor = wcolor;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
        if ([indicator respondsToSelector:@selector(setTintColor:)]) {
            [indicator setTintColor:wcolor];
        }
#endif
    }
}
...
//                _color = [UIColor colorWithWhite:0.0f alpha:0.8f];
//            } else {
//                _color = [UIColor colorWithWhite:0.0f alpha:0.8f];
//            }
//        } else {

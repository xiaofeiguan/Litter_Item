...
@dynamic firstLineIndent;

- (void)setFirstLineIndent:(NSNumber*)firstLineIndent
{
    objc_setAssociatedObject(self, @selector(firstLineIndent), firstLineIndent, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSNumber*)firstLineIndent
{
    id cur = objc_getAssociatedObject(self, @selector(firstLineIndent));
    return cur;
}
                             ...
                             range:(NSRange)range
{
    ...
    if ([self firstLineIndent]) {
        CTParagraphStyleSetting firstLineIndentStyle;
        CGFloat indent = [[self firstLineIndent] floatValue];
        firstLineIndentStyle.spec = kCTParagraphStyleSpecifierFirstLineHeadIndent;
        firstLineIndentStyle.value = &indent;
        firstLineIndentStyle.valueSize = sizeof(CGFloat);
        
        
        // 创建样式数组
        CTParagraphStyleSetting settings[] = {alignmentStyle ,lineSpaceStyle, paragraphSpaceStyle, lineBreakStyle, firstLineIndentStyle};
...
#pragma mark - 文本首行缩进属性
- (void)addAttributeFirstLineIndent:(CGFloat)indent
{
    [self setFirstLineIndent:@(indent)];

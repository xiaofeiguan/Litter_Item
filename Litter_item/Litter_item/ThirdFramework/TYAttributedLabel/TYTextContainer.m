...
- (void)setFirstLineIndent:(CGFloat)firstLineIndent
{
    if (_firstLineIndent != firstLineIndent) {
        ...
        [_attString addAttributeFirstLineIndent:firstLineIndent];
    }
}
...
- (BOOL)enumerateLinkRectContainPoint:(CGPoint)point viewHeight:(CGFloat)viewHeight successBlock:(void (^)(__strong id<TYLinkStorageProtocol> textStorage))successBlock
{
    return [self enumerateRunRect:_linkRectDictionary ContainPoint:point viewHeight:viewHeight successBlock:[successBlock copy]];
}
